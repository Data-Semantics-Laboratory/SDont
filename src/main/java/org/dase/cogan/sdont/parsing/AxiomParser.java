...
public class AxiomParser
{
	...
	private void parseObjectProperty(OWLObjectProperty objProp)
	{
		...
		{
			OWLClassExpression superClass = df.getOWLObjectSomeValuesFrom(objProp.asObjectPropertyExpression(),
			        range.getRange());
	...
	private void parseAxiom(OWLSubClassOfAxiom ax)
	{
		...
		try
		{
			...
			{
				sub.disjunctSet().forEach(u -> parseAxiom(df.getOWLSubClassOfAxiom(u, sup)));
			}
			else if(sup instanceof OWLObjectUnionOf)
			{
				sup.disjunctSet().forEach(d -> parseAxiom(df.getOWLSubClassOfAxiom(d, sub)));
			}
			else if(sup instanceof OWLObjectIntersectionOf)
			{
				sup.conjunctSet().forEach(c -> parseAxiom(df.getOWLSubClassOfAxiom(sub, c)));
			}
			else // weird stuff
			{
